...
async def totem(request: Request):
    # CORREÇÃO: O 'request' deve ser passado como argumento nomeado
...
async def painel(request: Request, db: Session = Depends(get_db)):
    ...
    # CORREÇÃO: Contexto agora é passado via 'context'
...
async def atendente_p(request: Request):
...
async def proxima(dados: schemas.ChamadaRequest, db: Session = Depends(get_db)):
    ...
    await manager.broadcast({"tipo": "atualizar_painel", "senha": p.codigo, "nome": p.nome, "guiche": p.guiche})
    await manager.broadcast({"tipo": "atualizar_lista"})
    return p
